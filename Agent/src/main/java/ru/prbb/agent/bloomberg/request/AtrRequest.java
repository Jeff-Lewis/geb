	...
	public void execute(String name) {
		...
		try {
			...
			for (String security : securities) {
				...
				historical.setElement("startDate", sdf.format(startDate)); // set
																			// study
																			// start
																			// date
				historical.setElement("endDate", sdf.format(endDate)); // set
																		// study
																		// start
																		// date
	...
	public void processMessage(Message sr) {
		...
		for (int j = 0; j < sdArray.numValues(); ++j) {
			...
			try {
				...
				log.error("Parse date string " + date, e);
			}
		}
	}
}
