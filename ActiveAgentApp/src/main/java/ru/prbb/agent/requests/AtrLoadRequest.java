 ...
 * @author RBr
 */
public class AtrLoadRequest extends BloombergRequest {
	...
	public void execute(String name) {
		start(name);
		try {
			final Service service = getService("//blp/tasvc");
			...
			for (String security : securities) {
				...
				historical.setElement("startDate", startDate); // set study
																// start date
				historical.setElement("endDate", endDate); // set study start
															// date
	...
	@Override
	public void process(Message sr) {
		// final Element sr = msg.getElement("studyResponse");

		final String security = sr.getElementAsString("securityName");

		/*
		 * studyResponse = { securityName = EAAPL US Equity studyError = {
		 * source = 210::bbdbh2 code = 15 category = BAD_SEC message =
		 * Unknown/Invalid securityInvalid Security [nid:210] subcategory =
		 * INVALID_SECURITY } fieldExceptions[] = { } studyData[] = { } }
		 */
		...
		/*
		 * studyResponse = { securityName = AAPL US Equity fieldExceptions[] = {
		 * } studyData[] = { studyData = { date = 2013-10-01T00:00:00.000+00:00
		 * ATR = 10.264273602783119 } studyData = { date =
		 * 2013-10-02T00:00:00.000+00:00 ATR = 9.710705202087341 } } }
		 */
