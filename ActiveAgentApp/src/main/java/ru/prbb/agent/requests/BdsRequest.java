 ...
 * @author RBr
 */
public class BdsRequest extends BloombergRequest {
	...
	public void execute(String name) {
		...
		try {
			...
			sendRequest(request, new CorrelationID());

			if (!peers.isEmpty()) {
				final Request requestP = createRequest("//blp/refdata", "ReferenceDataRequest");
				...
				sendRequest(requestP, new CorrelationID(peers));
			}
		} finally {
			stop();
		}
	...
	public static class PeerData {
		...
		public PeerData(String sec, Element fieldData) {
			this.sec = sec;
			this.cur_mkt_cap = BloombergRequest.getElementAsFloat64(fieldData, "CUR_MKT_CAP");
			this.oper_roe = BloombergRequest.getElementAsFloat64(fieldData, "OPER_ROE");
			this.bs_tot_liab2 = BloombergRequest.getElementAsFloat64(fieldData, "BS_TOT_LIAB2");
			this.pe_ration = BloombergRequest.getElementAsFloat64(fieldData, "PE_RATIO");
			this.ebitda = BloombergRequest.getElementAsFloat64(fieldData, "EBITDA");
			this.group = BloombergRequest.getElementAsString(fieldData, "INDUSTRY_GROUP");
			this.sub = BloombergRequest.getElementAsString(fieldData, "INDUSTRY_subGROUP");
		}
	}
