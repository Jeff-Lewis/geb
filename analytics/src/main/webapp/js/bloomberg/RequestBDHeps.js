...
(function() {
	...
	var stCodes = new Ext.data.JsonStore({
		autoDestroy : true,
		autoLoad : true,
		url : 'rest/RequestBDHeps/Securities.do',
		baseParams : {
			fundamentals : 1
		},
		// root : 'sec',
		fields : [ 'security_code', 'short_name', 'calculation_crncy',
				'indstry_group', 'all_flag', 'portfolio', 'wl_flag', 'pivot',
				'new_flag' ],
		sortInfo : {
			field : 'short_name'
		},
	...
	var filter = new Ext.form.ComboBox({
		...
		store : new Ext.data.JsonStore({
			autoDestroy : true,
			url : 'rest/RequestBDHeps/EquitiesFilter.do',
			// root : 'info',
			fields : [ 'name' ],
			sortInfo : {
				field : 'name'
	...
	var equities = new Ext.form.ComboBox({
		...
		store : new Ext.data.JsonStore({
			autoDestroy : true,
			url : 'rest/RequestBDHeps/Equities.do',
			// root : 'info',
			fields : [ 'id', 'name' ],
			sortInfo : {
				field : 'name'
	...
	var periodSelect = new Ext.form.ComboBox({
		...
		store : new Ext.data.JsonStore({
			autoDestroy : true,
			url : 'rest/RequestBDHeps/Period.do',
			// root : 'info',
			fields : [ 'name' ]
		}),
		editable : false,
	...
	var calendarSelect = new Ext.form.ComboBox({
		...
		store : new Ext.data.JsonStore({
			autoDestroy : true,
			url : 'rest/RequestBDHeps/Calendar.do',
			// root : 'info',
			fields : [ 'name' ]
		}),
		editable : false,
	...
	return new Ext.Panel({
		id : 'RequestBDHeps-component',
		title : 'BDH запрос с EPS',
		frame : true,
		closable : true,
		layout : 'border',
