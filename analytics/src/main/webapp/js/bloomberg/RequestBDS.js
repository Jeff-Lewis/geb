 ...
 * BDS запрос
 * 
 * у блума есть тип полей, которые называются bulk они возвращают сразу какой-то
 * набор данных. в нашем случае bloomberg_peers. этот параметр возвр. список
 * конкурентов компании. получив этот список ты должен его сохранить в бд с
 * помощью хп dbo.put_blmpeers_proc помимо этого, нужно еще получить некий
 * дескрипшн этих конкурентов. для этого выполняется bdp запрос по каждой из
 * компаний, которую ты получил на пред. этапе. далее нужно сохранить этот
 * дескрипшн с помощью хп dbo.put_blmpeers_descr_proc т.к. набор конкурентов
 * компании намного больше набора компаний, которые мы просматриваем, этот
 * дескрипшн я храню в отдельной таблице. в противном случае пришлось бы
 * добавлять каждого конкурента через форму добавления компании, что немного
 * утомительно для такого кол-ва. по этому нельзя при закачке пирсов юзать один
 * в один запрос бдп
 * 
 */
(function() {
